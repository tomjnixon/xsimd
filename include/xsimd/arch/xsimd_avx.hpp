...
namespace xsimd {
  ...
  namespace kernel {
    ...
    namespace detail {
    inline void split_avx(__m256i val, __m128i& low, __m128i& high) {
    ...
    template<class A> batch<float, A> fast_cast(batch<uint32_t, A> const& v, batch<float, A> const&, requires_arch<avx>) {
      ...
      __m256i v_lo      = _mm256_and_si128(v,msk_lo);          /* extract the 16 lowest significant bits of self                             */
    ...
    batch<T, A> zip_hi(batch<T, A> const& self, batch<T, A> const& other, requires_arch<avx>) {
      switch(sizeof(T)) {
        ...
        case 4: return _mm256_unpackhi_epi32(self, other);
        case 8: return _mm256_unpackhi_epi64(self, other);
    ...
    batch<T, A> zip_lo(batch<T, A> const& self, batch<T, A> const& other, requires_arch<avx>) {
      switch(sizeof(T)) {
        ...
        case 4: return _mm256_unpacklo_epi32(self, other);
        case 8: return _mm256_unpacklo_epi64(self, other);
        default: assert(false && "unsupported arch/op combination"); return {};
      }
    }
