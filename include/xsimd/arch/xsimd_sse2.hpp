...
namespace xsimd {
  ...
  namespace kernel {
    ...
    template<class A> batch<float, A> fast_cast(batch<uint32_t, A> const& v, batch<float, A> const&, requires_arch<sse2>) {
    ...
    batch<T, A> sadd(batch<T, A> const& self, batch<T, A> const& other, requires_arch<sse2>) {
      if(std::is_signed<T>::value) {
        switch(sizeof(T)) {
          ...
          default: return detail::sadd_default(self, other, A{});
        }
      }
      else {
        switch(sizeof(T)) {
    ...
    batch<T, A> ssub(batch<T, A> const& self, batch<T, A> const& other, requires_arch<sse2>) {
      if(std::is_signed<T>::value) {
        switch(sizeof(T)) {
          ...
          default: return detail::ssub_default(self, other, A{});
        }
      }
      else {
        switch(sizeof(T)) {
